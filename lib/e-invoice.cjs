...
function normalizeInvoiceForEInvoice(invoice) {
  ...
  return {
    ...
    seller: {
      identifier: asString(invoice.companyVatId || invoice.companyTaxNumber),
...
function validateEInvoiceData(invoice) {
  ...
  if (!normalized.seller.identifier) missingFields.push("Verkaeuferkennung (Steuernummer oder USt-IdNr.)");
  ...
  <rsm:SupplyChainTradeTransaction>
    ...
    <ram:ApplicableHeaderTradeAgreement>
      ...
      <ram:SellerTradeParty>
        ${normalized.seller.identifier ? `<ram:ID>${escapeXml(normalized.seller.identifier)}</ram:ID>` : ""}
