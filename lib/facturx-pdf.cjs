...
async function createPdfBoxCarrierPdf(outputPdfPath, options = {}) {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "facturx-carrier-"));
  try {
    ...
    await execFileAsync(
      "java",
      ["-cp", classPath, "tools.facturx.FacturXPdfBoxEmbedder", "--carrier", outputPdfPath],
      { timeout: 120000, cwd: baseDir }
    );
    await assertFileNonEmpty(outputPdfPath, "HSRechnung-Traeger-PDF wurde nicht erstellt.");
  } finally {
